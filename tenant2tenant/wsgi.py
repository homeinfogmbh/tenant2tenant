...
SKIPPED_PATCH_FIELDS = set(
    key
    for key, *_ in TenantMessage.get_json_fields()
    if key not in ALLOWED_PATCH_FIELDS
)


def _get_messages(
    customer: Union[Customer, int], released: Union[bool, None]
) -> Select:
...
def _get_message(ident: int) -> TenantMessage:
    ...
    return (
        TenantMessage.select(cascade=True)
        .where((TenantMessage.id == ident) & (TenantMessage.customer == CUSTOMER.id))
        .get()
    )
...
def list_messages() -> JSON:
    ...
    return JSON(
        [message.to_json() for message in _get_messages(CUSTOMER.id, _get_released())]
    )


@APPLICATION.route("/message/<int:ident>", methods=["GET"], strict_slashes=False)
@authenticated
@authorized("tenant2tenant")
...
@APPLICATION.route("/message/<int:ident>", methods=["PUT"], strict_slashes=False)
@authenticated
@authorized("tenant2tenant")
def toggle_message(ident: int) -> JSONMessage:
    ...
    message.save()
    return JSONMessage(
        "The message has been toggled.", released=message.released, status=200
    )


@APPLICATION.route("/message/<int:ident>", methods=["PATCH"], strict_slashes=False)
@authenticated
@authorized("tenant2tenant")
...
@APPLICATION.route("/message/<int:ident>", methods=["DELETE"], strict_slashes=False)
@authenticated
@authorized("tenant2tenant")
...
def get_config() -> Union[JSON, JSONMessage]:
    ...
    configuration = (
        ...
        .where(Configuration.customer == CUSTOMER.id)
        .get()
    )
...
APPLICATION.add_routes([("GET", "/email", GET_EMAILS), ("POST", "/email", SET_EMAILS)])
...
def handle_missing_configuration(_: Configuration.DoesNotExist) -> JSONMessage:
    ...
    return JSONMessage("The requested configuration does not exist.", status=404)
